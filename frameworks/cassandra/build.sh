...
if [ x"$cli_only" = xtrue ]; then
    ${ROOT_DIR}/tools/build_framework.sh --cli-only $PUBLISH_STEP cassandra-portworx $FRAMEWORK_DIR $BUILD_DIR/executor.zip $BUILD_DIR/cassandra-scheduler.zip
    exit $?
fi

${ROOT_DIR}/tools/build_framework.sh $PUBLISH_STEP cassandra-portworx $FRAMEWORK_DIR $BUILD_DIR/executor.zip $BUILD_DIR/cassandra-scheduler.zip
